...
int http_get(HTTP_INFO *hi, char *url, char *response, int size)
{
    ...
    {
        char authToken[strlen(hi->request.authorization) + 1];
        strcpy(authToken, hi->request.authorization);
        https_close(hi);

        https_init(hi, https, verify);
        strcpy(hi->request.authorization, authToken);
...
//          printf("socket reuse: %d \n", sock_fd);
    ...
    len = snprintf(request, 1024,
            ...
            "%s\r\n"
            "%s",
            method, dir, host, port,
            (int)strlen(data),
            hi->request.authorization,
            hi->request.cookie,
            data);
