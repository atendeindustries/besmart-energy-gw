...
static int http_open(HTTP_INFO* hi) {
    int ret;
    ...
    if (ret != 0) {
        if (DEBUG) printf( " failed! mbedtls_net_connect returned -0x%x\n\n", abs(ret) );
        return ret;
    }
    ...
    if (hi->ssl > 0) {
        ...
        if(ret != 0) {
            if (DEBUG) printf( " failed! mbedtls_ssl_config_defaults returned -0x%x\n\n", abs(ret));
            return ret;
        }
        ...
        if( ret != 0 ) {
            if (DEBUG) printf( " failed! mbedtls_ssl_setup returned -0x%x\n\n", abs(ret));
            return ret;
        }

        ret = mbedtls_ssl_set_hostname( &hi->httpSSL.ssl, hi->host );
        if( ret != 0 ) {
            if (DEBUG) printf( " failed! mbedtls_ssl_set_hostname returned -0x%x\n\n", abs(ret));
            return ret;
        }
        ...
        while ((ret = mbedtls_ssl_handshake(&hi->httpSSL.ssl)) != 0)
        {
            ...
            {
                if (DEBUG) printf( " failed! mbedtls_ssl_handshake returned -0x%x\n\n", (unsigned int) -ret );
                return ret;
            }
        }
    }
...
static int http_close(HTTP_INFO* hi) {
    int ret;
